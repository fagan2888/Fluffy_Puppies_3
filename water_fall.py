...
def SMM_D(B, H): ##Need to be updated as expoential of negative integral of LTV
    return 0.01
    
def waterfall_helper(P, B_tr, flag = 'desc'):
    R = P
    if(R > sum(B_tr)):
...
def water_fall(B_F_t, FRM_monthly_wac, B_A_t, Libor_t, House_F_m_t, House_A_m_t, B_tr, B_F_0_ = B_F_0, B_A_0_ = B_A_0):
    ...
    P_F = PM_F - I_F
    PP_F = SMM_P[j] * (B_F_t - P_F)
    DP_F = SMM_D(B_F_t, House_F_m_t) * (B_F_t - P_F)
    ...
    P_A = PM_A - I_A
    PP_A = SMM_P[j] * (B_A_t - P_A)
    DP_A = SMM_D(B_A_t, House_A_m_t) * (B_A_t - P_A)
    ...
    for k in range(num_tr):
        I_tr[k] = B_tr[k]*(Libor_m[i,j]+Spread_tr[k])/12
    ES = I - sum(I_tr) #Excess Spread
    ES_init = ES
    if (ES<0):
        print("ES<0, setting ES = 0")    
        ES = 0
    if (ES>DP):
        ES -= DP
        DP = 0
...
Libor_m = np.zeros((Path,J))+0.01
House_F_m = np.zeros((Path,J))+0.01
House_A_m = np.zeros((Path,J))+0.01
SMM_P = np.zeros(J) + 0.01
